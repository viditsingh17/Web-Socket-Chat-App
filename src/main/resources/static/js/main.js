...
//Assign different colours to different users
var colors = [
    '#2196F3', '#32c787', '#00BCD4', '#ff5652',
    '#ffc107', '#ff85af', '#FF9800', '#39bbb0'
];

//Allows a user to connect to the session
function connect(event) {
    username = document.querySelector('#name').value.trim();

    if(username) {
        //if valid username - switch views
...
function onConnected() {
    ...
    // TODO: subscribe to a topic of user's choice
...
function sendMessage(event) {
    //TODO : add image sharing functionality 
...
function encodeImageToBase64(file, callback) {
    ...
    reader.onload = function () {
        callback(reader.result.split(",")[1]); // Base64 string
    };
...
//We call the connect method once the submit button is pressed
usernameForm.addEventListener('submit', connect, true);

messageForm.addEventListener('submit', sendMessage, true);

messageForm.addEventListener('image', sendMessage, true);